...
class AssertVerdictReporter {
  ...
  void AssertResult_() { ASSERT_TRUE(has_overall_result_ && (!require_scoring_ || has_scoring_result_)); }
...
inline void PrintTo(const Verdict& verdict, std::ostream* os) { *os << VerdictToAbr(verdict); }

long SetupSubmission(Submission& sub, int id, Compiler lang, long time, bool sandbox_strict,
                     const std::string& code, SpecjudgeType spec_type = SpecjudgeType::NORMAL,
                     const std::string& specjudge_code = "", SummaryType summary_type = SummaryType::NONE,
                     const std::string& summary_code = "", const std::string& hack_code = "",
                     int submitter_id = 10);
