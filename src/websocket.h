...
class WsClient {
 ...
 private:
  ...
  TLSContextPtr OnTLSInit_(Handle) {
    ...
    ctx->set_options(context::default_workarounds | context::no_sslv2 | context::no_sslv3 |
  ...
  void OnOpen_(Handle hdl) {
    connected_ = true;
    hdl_ = hdl;
    std::visit([&](auto& client) { OnOpen(client.get_con_from_hdl(hdl)->get_response()); }, client_);
  }
  void OnFail_(Handle) { OnFail(); }
  ...
  template <class Client>
  void OnMessage_(Handle, typename Client::message_ptr msg) {
    OnMessage(msg->get_payload());
  }

  template <class Client>
  bool Connect_(Client& c) {
    using namespace websocketpp::lib;
    error_code ec;
    ...
    conn->set_message_handler(
        bind(&WsClient::OnMessage_<Client>, this, placeholders::_1, placeholders::_2));
    c.connect(conn);
    return true;
  }

  template <class Client>
  bool Send_(Client& c, const std::string& str) {
